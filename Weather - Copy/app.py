...
def pred():
    ...
    city = request.form['city'] if request.method == 'POST' else 'Lucknow'

    # API request to get weather data
    url = f'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    ...
    SEARCH_ENGINE_ID = 'xxxxxxxxxxxxxxxxxxxxx'
    query = f"{city} 1920x1080"
    page = 1
    start = (page - 1) * 10 + 1
    searchType = 'image'
    # Construct URL for Google Custom Search API request to get image URL
    city_url = f"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
...
def send_report_message(message):
    # Send SMS
    client.messages.create(to="xxxxxxxxxxxxxxxxxxx", from_=twilio_phone_number, body=message)
